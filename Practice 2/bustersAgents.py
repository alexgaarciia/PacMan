...
class QLearningAgent(BustersAgent):
    ...
    def computePosition(self, state):
        ...
        """
        current_state = self.computeState(state)
        return current_state[1] * 4 + current_state[0]

    def computeState(self, state):
        pacman_pos = state.getPacmanPosition()
        ghost_pos = (self.closest_ghost(state)[1], self.closest_ghost(state)[2])

        # COMPUTE WHERE IS THE GHOST
        if ghost_pos[0] < pacman_pos[0] and ghost_pos[1] < pacman_pos[1]:
            location_of_ghost = 2  # third quadrant, with respect to pacman
        elif ghost_pos[0] < pacman_pos[0] and ghost_pos[1] >= pacman_pos[1]:
            location_of_ghost = 1  # second quadrant, with respect to pacman
        elif ghost_pos[0] >= pacman_pos[0] and ghost_pos[1] < ghost_pos[1]:
            location_of_ghost = 3  # fourth quadrant, with respect to pacman
        else:
            location_of_ghost = 0  # first quadrant, with respect to pacman

        # NOW COMPUTE HOW CLOSE IS THE DISTANCE
        distancer = Distancer(state.data.layout)
        distance_now = distancer.getDistance(pacman_pos, ghost_pos)

        if distance_now <= 2:
            distance = 0
        elif 2 < distance_now < 8:
            distance = 1
        elif 8 <= distance_now < 13:
            distance = 2
        else:
            distance = 3

        return (location_of_ghost, distance)
    ...
    def getWalls_and_Pacdots(self, state):
        ...
        return (total_walls, food)  # these are lists that contain the coordinates of all the walls
